...
from collector.shared.db import Company, create_tables, get_session
...
def _load_employment_overrides() -> dict[str, float]:
    ...
    """
    csv_path = Path(__file__).resolve().parents[3] / "kb" / "employment_overrides.csv"
...
def _find_gnem_excel() -> Path:
    ...
    candidates = [
        ...
        Path(__file__).resolve().parents[3] / "ev_data_LLM_comparsions" / "GNEM - Auto Landscape Lat Long Updated.xlsx",
        Path(__file__).resolve().parents[3] / "GNEM - Auto Landscape Lat Long Updated.xlsx",
    ]
