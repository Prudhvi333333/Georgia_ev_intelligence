...
from collector.phase2_embedding.chunker import chunk_company_record, chunk_document, get_child_chunks
from collector.phase2_embedding.embedder import embed_single, embed_texts, verify_ollama_embed
from collector.phase2_embedding.vector_store import (
...
from collector.phase1_extraction.kb_loader import build_document_text, get_all_companies_from_db
from collector.phase2_embedding.embedder import embed_chunks
...
def test_search_5_queries():
    ...
    assert hits >= 1, (
        ...
        f"Run Phase 2 pipeline first: venv\\Scripts\\python -m collector.phase2_embedding.pipeline --companies-only"
    )
...
def main():
    ...
    if passed == total:
        ...
        print(f"     venv\\Scripts\\python -m collector.phase2_embedding.pipeline --companies-only")
        print(f"     venv\\Scripts\\python -m collector.phase2_embedding.pipeline")
    else:
