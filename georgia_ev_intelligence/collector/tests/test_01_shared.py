...
class TestDatabaseConnection(unittest.TestCase):
    ...
    def test_connection_is_alive(self):
        from collector.shared.db import verify_connection
    ...
    def test_tables_can_be_created(self):
        from collector.shared.db import create_tables
        # Should not raise
        create_tables()

    def test_company_table_schema(self):
        from collector.shared.db import create_tables, get_engine
...
class TestB2Connection(unittest.TestCase):
    ...
    def test_connection_is_alive(self):
        from collector.shared.storage import verify_connection
        ok = verify_connection()
        self.assertTrue(ok, "Backblaze B2 connection failed — check B2_* keys in .env")

    def test_key_naming_convention(self):
        from collector.shared.storage import make_document_key
