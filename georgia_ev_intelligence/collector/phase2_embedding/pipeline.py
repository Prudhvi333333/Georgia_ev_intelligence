...
Usage:
  ...
  venv\\Scripts\\python -m collector.phase2_embedding.pipeline --company "Hanwha Q Cells"

  # Re-embed (overwrite existing):
  venv\\Scripts\\python -m collector.phase2_embedding.pipeline --reembed
"""
...
from collector.phase2_embedding.chunker import (
    Chunk,
    chunk_company_record,
    chunk_document,
    get_child_chunks,
)
from collector.phase2_embedding.embedder import embed_chunks, verify_ollama_embed
...
from collector.shared.db import (
...
def _save_chunk_tracking(
    ...
    for chunk in chunks:
        ...
        from collector.shared.db import DocumentChunk
        dc = DocumentChunk(
...
def _fetch_document_text(doc_id: int, b2_key: str | None, session) -> str | None:
    ...
    try:
        import collector.shared.storage as storage_mod
