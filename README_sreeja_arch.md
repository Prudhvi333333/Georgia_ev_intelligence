...
| `evaluate/format_runner.py` | **Not imported.** | Loads `Config.get()` at module import time, which would force unrelated env vars to be present. We re-implement only the small Tavily call here. |
...
| Qdrant collection | **Reuse existing data.** | Pipeline targets `georgia_ev_chunks__nomic_embed_text` (set via `QDRANT_COLLECTION_NAME` in `.env`). |
...
georgia_ev_intelligence/scripts/
  run_llm_comparison.py   # generation CLI
...
5. **Smoke-test the reranker** (catches a bad install before you spend on LLM calls):
   ...
   python -c "from llm_comparison.retrieval import ensure_reranker; \
              ...
              ensure_reranker(load_generation_config().reranker_model); \
              print('reranker OK')"
   ```
...
| `QDRANT_COLLECTION_NAME` | Collection to query. Use `georgia_ev_chunks__nomic_embed_text` (existing data). | The Phase-2 ingest pipeline created this. |
...
Generation and evaluation are deliberately separate commands — you can use a
different model for evaluation than for generation.

### Command 1 — full run, all 50 questions × 2 models × 4 modes

```bash
python -m scripts.run_llm_comparison \
  --run-id full_$(date +%Y%m%d) \
  --models qwen2.5:14b gemma3:27b \
  --modes rag_only no_rag rag_pretrained rag_pretrained_web

python -m scripts.run_llm_evaluation \
  --run-id full_<same_id_as_above> \
...
python -m scripts.run_llm_evaluation \
  --run-id subset_<same_id_as_above> \
  --judge-model "$JUDGE_MODEL"
```

Both scripts must be run from the `georgia_ev_intelligence/` directory (or
adjust `PYTHONPATH` so `scripts.run_llm_comparison` is importable). To run
from the repo root instead:

```bash
PYTHONPATH=georgia_ev_intelligence python -m scripts.run_llm_comparison ...
PYTHONPATH=georgia_ev_intelligence python -m scripts.run_llm_evaluation ...
```
...
`retrieved_count`, `rerank_top_n`, `generation_elapsed_s`, `embedding_model`,
...
- `run_metadata` — single row with run_id, models, modes, judge model, etc.
